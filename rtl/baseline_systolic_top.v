// baseline_systolic_top.v — N=32, K=64, STREAMS=8 — same fix applied

`include "token_pkg.vh"

module baseline_systolic_top #(
    parameter N        = 32,
    parameter K        = 64,
    parameter STREAMS  = 8,
    ...
    systolic_array #(.N(N),.K(K),.STREAMS(STREAMS),.ACC_W(ACC_W)) u_array (
        .clk(clk), .rst(rst),
        .A_in_flat(A_tok_flat), .B_in_flat(B_tok_flat),
        .psum_out_flat(psum_out_flat), .done_out_flat(done_out_flat)
    );
